...
weighted_stats <- plyr_rt_1720 %>%
                mutate_at(vars(c("g", "a", "pts", "ppg", "ppa", "s", "blk", "hit", "pim", "fow", "fol")), ~.*weights) %>%
                 mutate(toi_weight = toi*weights)
                
# We have the lg avg and the players stats. Now we must combine the 2. Then we need to find the expected rates of production for the minutes a player played. 
...
#fix age, make sure it's the right age for the aplyers
...
Projected F TOI = (n-1TOI * .52) + (n-2TOI * .21) + (200)
Projected D TOI = (n-1TOI * .50) + (n-2TOI * .23) + (250)




https://baseballwithr.wordpress.com/2014/06/25/marcel-the-matrix/